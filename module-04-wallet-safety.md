# Module 4: Using a Compatible Wallet Safely

*Part of the CeloHT Education curriculum — see [../EDUCATION.md](../EDUCATION.md).*
*Previous: [Module 3 →](./module-03-usdm-and-celo.md)*

## Audience
Individuals who have completed Module 3, immediately before their first real wallet setup.

## Duration
Approx. 60 minutes, hands-on (bring your own device).

## Learning Objectives

1. Set up a compatible wallet correctly
2. Understand what a seed phrase is and why it must never be shared
3. Verify a transaction before confirming it
4. Recognize the difference between the legitimate CeloHT dApp and a phishing imitation

## Lesson 4.1 — Choosing Your Wallet

Explain that the principles in this module apply to compatible wallets generally. For a concrete example, refer participants to the optional [Valora guide](./VALORA.md) for setup guidance.

## Lesson 4.2 — Setting Up Your Wallet (Hands-On)

Step-by-step, supervised setup. Facilitators watch for and correct common errors: skipping seed phrase backup, screenshotting the seed phrase (insecure), or entering the seed phrase into any website or chat.

## Lesson 4.3 — The Seed Phrase Rule

**Taught as an absolute, memorable rule, repeated multiple times in the session:**

> "Nobody from CeloHT, an agent, or any legitimate service will ever ask for your seed phrase. Ever. If someone asks, it is a scam — even if they say they're 'from CeloHT support.'"

This directly operationalizes [../SECURITY.md](../SECURITY.md#wallet-safety).

## Lesson 4.4 — Verifying a Transaction

Hands-on walkthrough: what to check before approving a transaction in the wallet app — recipient address, amount, and network fee — matching what was agreed in person (relevant directly to Agent Network cash-in/cash-out, see [../AGENT_NETWORK.md](../AGENT_NETWORK.md#agent-interaction)).

## Lesson 4.5 — Recognizing Phishing

Show real (redacted) examples of the difference between the legitimate CeloHT dApp URL/app and common phishing patterns (lookalike URLs, fake "urgent action required" messages, unsolicited DMs offering help). Cross-reference [../SECURITY.md](../SECURITY.md#threat-model).

## Assessment

Practical: participant successfully completes a supervised testnet transaction and correctly identifies 3 out of 3 example scenarios as either safe or a scam attempt.

## Next Module

[Module 5: Digital Security →](./module-05-digital-security.md)
